...
int main() {
  ...
  //     graphVisualization << "\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
  //     << endl; graphVisualization << "origin: " << origin.first.stop << endl;
  //     for (pair<const Vertex, vector<Edge>> destination : origin.second) {
  //         graphVisualization << "\n\tdestination: " << destination.first.stop
  //         << endl; for (size_t i = 0; i < destination.second.size(); i++) {
  //             graphVisualization << "\t\tedge: {" <<
  //             destination.second[i].route << ", " <<
  ...
  //                         "?\t\t";
  //                 bool result = algorithm.RouteConnection(route, v1.stop,
  //                 v2.stop); dfs << (result ? "YES." : "NO.") << endl << endl;
  //             }
  //         }
  //     }
  // }
  ...
  string route =
      "[@6.0.28929208@][32][1657992971959]/6__7W_#2_SHOW_HS_Early_Out";
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      ...
      if (origin != destination) {
        dfs_less_route << "Is there a connection between " << origin << " and "
                       << destination << " through route " << route << "?\t\t";
  ...
  cout << "\nRouteConnection() is done; let's move on to Tarjan's Algorithm."
       << endl;
  ...
  cout << "\nTarjan() is done; let's take a look at what on to Prims's "
          "Algorithm tells us about our data."
       << endl;
